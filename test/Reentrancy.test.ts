...
describe("Reentrancy", () => {
	...
	it("Should increase the balance of the contract", async () => {
		...
		expect((await provider.getBalance(Reentrancy.address))).to.equal(ethers.BigNumber.from(0));
		...
		expect((await provider.getBalance(Reentrancy.address))).to.equal(ethers.utils.parseEther("1.0"));
	...
	it("Should be reverted because it is not called by the owner", async () => {
		...
		await expect(Reentrancy.connect(addr1).withdraw(owner.address, ethers.utils.parseEther("1.0")))
			.to.be.reverted;
